...
Neural Networks, just like a human brain, are made up of component pieces. The similarities mostly end there, as the pieces of a neural network, though inspired by the biological brain, are fundamentally different.

### Neurons

**Neurons** form the basis of neural networks and are where they get their name. In code, a neuron is often referred to as a **unit**.

A neuron is an analogue to a biological neuron, although in my opinion, even though their human cousins may have served as the initial inspiration, they're so fundamentally different that its better to think of them as totally a new thing.

[IMAGE OF A NEURON]

### Layers

A collection of neurons makes up a **layer**.
...
Deep Learning requires more data, is slower to train, and produces highly uninterpretable models. However, the one use case in which they shine - and which encompass a startinlingly large array of problems - is around nonlinear data.
...
On the other hand, it introduces complexity into the code, and makes it more difficult to package and send around your model. (They're not portable).
